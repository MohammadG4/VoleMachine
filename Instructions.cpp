...
void Instructions::readFromFile() {
    ...
    cout << "Please enter the file name or path: ";
    cin >> txt;
    file.open(txt);
    while (!file.is_open()) {
        cout << "Error Opening file!\nPlease enter the file name or path: ";
        cin >> txt;
        file.open(txt);
    }
    string line;
...
void printSteps(Memory Storage, Register Processor) {
    cout << "Memory :\n\n";
    for (int j = 0; j < 256; j++) {
...
//C000
void Instructions::decode(Register &registers, Memory &memory, int &programCounter, int start, bool printWhole) {
